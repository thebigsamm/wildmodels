...
export async function POST(req: Request) {
  ...
  const { data: profile, error } = await supabaseAdmin
    .from("profiles")
    .select(
      "id, username, display_name, gender, orientation, age, city, area, bio, photo_url, whatsapp, telegram, interests, status, is_active, is_hidden_by_owner, deleted_at, created_at, rejection_count, approved_snapshot"
    )
    .eq("id", profileId)
    .maybeSingle();
