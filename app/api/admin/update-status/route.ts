...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: profile, error: fetchErr } = await supabaseAdmin
      .from("profiles")
      .select(
        "id, user_id, username, display_name, orientation, age, city, area, bio, whatsapp, telegram, price_short_time, price_overnight, price_weekend, status, first_approved_at, rejection_count"
      )
      .eq("id", id)
      .maybeSingle();
